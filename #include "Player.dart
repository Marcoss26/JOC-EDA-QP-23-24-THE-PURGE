    ...
    /*void make_move_op( const Mapa& map, int id, Pos p_bw, Pos p_fin, bool& moved ){
      ...
      if(p_fin.i < p_bw.i) {
        ...
        moved = true;
          ...
          pl.j -= 1;
          int r = random(1,2);
          if(r == 1 and pos_ok(pr) and cell(pr).type == Street) {move(id,Right);moved = true; return;}
          if(r == 2 and pos_ok(pl) and cell(pl).type == Street) {move(id,Left);moved = true; return;}
        }
      ...
      if(p_fin.j < p_bw.j) { 
        ...
        if(pos_ok(new_pos) and cell(new_pos).type == Street ){
          ...
          pd.i += 1;
          int r = random(1,2);
          if(r == 1 and pos_ok(pu) and cell(pu).type == Street) {move(id,Up);moved = true; return;}
          if(r == 2 and pos_ok(pd) and cell(pd).type == Street) {move(id,Down); moved = true; return;}
        }
